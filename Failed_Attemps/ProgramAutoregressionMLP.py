...
class Program:
    def Start(self):
        ...
        # generates a relative history index matrix (e.g., [-5/30s, -4/30s, -3/30s, -2/30s, -1/30s])
        self.HistoryOffsets = torch.arange(-WINDOW_SIZE, 0) / FRAMERATE

        # tracks rolling runtime history container for live interaction
        self.EditorHistory = torch.zeros(1, WINDOW_SIZE, FRAME_DIM)

        self.Trainer = self.Training()

    # generates the scene and actor on the 3d environment
    def Standalone(self): 
    ...
    def Draw(self):
        with torch.no_grad():
            current_frame = self.GetEditorFeatures().unsqueeze(0).unsqueeze(1) #.to(Tensor.Device) # [1, 1, FRAME_DIM]

            # deletes the first frame from the history window and adds current frame (like sliding the window)
