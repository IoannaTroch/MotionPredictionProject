...
class Model(nn.Module):
    def __init__(self, frame_dim, window_size, hidden_dim, dropout=Defaults.Dropout):
        ...
        # Autoregressive Input Dimension = (Number of past frames) x (Features per frame)
        self.InputDim = window_size * frame_dim
        self.OutputDim = frame_dim # It predicts exactly ONE future frame
    ...
    def forward(self, history, generate_steps=1):
        ...
        # Clone history to prevent modifying original data tensors
        current_history = history.clone()

        for _ in range(generate_steps):
            # Flatten the sliding window: [Batch, WindowSize, FrameDim] -> [Batch, InputDim]
            x = current_history.reshape(batch_size, self.InputDim)
            
            # Standard MLP processing pipeline with normalization
            ...
            # SLIDE THE WINDOW (Autoregressive Feedback loop)
            # Drop the oldest frame (index 0) and append the new prediction to the end
            current_history = torch.cat([current_history[:, 1:, :], next_frame.unsqueeze(1)], dim=1)

        # Stack predictions along the time axis: [Batch, generate_steps, FrameDim]
    ...
    def learn(self, input_windows, target_frames, update_stats):
        ...
        # Standard supervised regression training pass
