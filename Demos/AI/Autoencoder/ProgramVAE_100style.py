...
class Program:
    ...
    def Standalone(self):#zografizei to perivalon an exw standalone
        self.Editor = AI4Animation.Scene.AddEntity("Editor").AddComponent(
            MotionEditor,
            self.Dataset,
            os.path.join(ASSETS_PATH, "100style_character.glb"),
            BONES,
        )
        self.Actor = AI4Animation.Scene.AddEntity("Actor").AddComponent(
            Actor, os.path.join(ASSETS_PATH, "100style_character.glb"), BONES
        )
    ...
    def Training(self):
        ...
        torch.save(self.Network, "vae_full_model_100style.pth")
