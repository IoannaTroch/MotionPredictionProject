...
LATENT_DIM = 256         #256 latent apo vae
HIDDEN_DIM = 512 
WINDOW_SIZE = 5 
...
class Program:
    def Start(self):
        ...
        #fortwsh pre-trained vae
        print("Loading pre-trained VAE...")
        
        if os.path.exists("vae_full_model.pth"): #koitaw an exei apothikeutei olo to montelo
        ...
        self.VAE.eval()  #vale to montelo se eval kai pagwse ta varh
    ...
    def EncodeBatch(self, raw_data, window_size):
        ...
        #pernane ola ta kare apo ton vae
        raw_reshaped = raw_data.view(-1, FRAME_DIM)

        #raw_tensor = Tensor.ToDevice(torch.tensor(raw_reshaped, dtype=torch.float32))
        ...
        mu = self.VAE.fc_mu(hidden) # pairnoume mono mesh timh, katharh plhroforia (san to validation)
        #kanw resize sto swsto sxhma
    ...
    def Training(self): 
        ...
        for epoch in range(1, EPOCH_COUNT + 1):
            ...
            for i, batch_data in enumerate(train_batches):
                xBatch_raw = batch_data[0] # Παρελθόν (Raw)
                yBatch_raw = batch_data[1] # Μέλλον (Raw)
                
                #metatroph raw data se latent
                with torch.no_grad():
                    xBatch_latent = self.EncodeBatch(xBatch_raw, WINDOW_SIZE)
                    yBatch_latent = self.EncodeBatch(yBatch_raw, 1)

                #ekpaideuw lstm se latent data
    ...
    def PlotTrainVal(self, train_losses, val_losses, epoch):
        ...
        plt.title(f'Latent LSTM MSE Loss (Epoch {epoch}/{EPOCH_COUNT})')
        plt.xlabel('Epoch')
    ...
    #eksagei xarakthristika sto raw xwro
    ...
    def Draw(self):
        with torch.no_grad():
            ...
            #o vae kanei decode thn epomenh raw kinhsh apo thn latent space provlepsei tou LSTM
